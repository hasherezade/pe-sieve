...
namespace pesieve {
	...
	public:
		...
		class AreaInfo {
		public:
			static AreaInfo* fillInfo(HANDLE processHandle, bool isReflection, ULONGLONG susp_addr);

			AreaInfo(ULONG_PTR _base, size_t _regionSize, DWORD _allocationProtect)
				: base(_base), regionSize(_regionSize), allocationProtect(_allocationProtect)
			{
			}

			bool fillStats(HANDLE processHandle, bool isReflection);
		...
		~ThreadScanReport()
		{
			for (auto itr = suspiciousAreas.begin(); itr != suspiciousAreas.end(); ++itr) {
		...
		const virtual void fieldsToJSON(std::stringstream &outs, size_t level, const pesieve::t_json_level &jdetails)
		{
			...
			AreaInfo* info = getInfoForAddress((ULONG_PTR)this->module);
			if (info) {
				outs << ",\n";
				OUT_PADDED(outs, level, "\"protection\" : ");
				outs << "\"" << std::hex << protection << "\"";
				if (info->stats.isFilled()) {
					outs << ",\n";
					info->stats.toJSON(outs, level);
				}
			}
		}

		AreaInfo* getInfoForAddress(ULONG_PTR address)
		{
			AreaInfo* info = nullptr;
			for (auto itr = suspiciousAreas.begin(); itr != suspiciousAreas.end(); ++itr) {
				AreaInfo* _info = itr->second;
				if (!_info) continue;
				if (susp_addr >= _info->base && susp_addr < (_info->base + _info->regionSize)) {
					info = _info;
					break;
				}
			}
		...
		AreaInfo* makeInfoForAddress(HANDLE processHandle, bool isReflection, ULONG_PTR address);
		...
		std::map<ULONG_PTR, AreaInfo*> suspiciousAreas;
	};
